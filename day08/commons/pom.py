...
import day07.commons.utils as utils
from day07.commons.utils import is_front_login
...
class BackgroundLoginPage(BasePage):
    ...
    def login(self,username,password):
        ...
        if not utils.is_back_login(self.driver):
            ...
            self.find_element(*self.ipt_password).send_keys(password)
            # ○ 输入验证码
            self.find_element(*self.ipt_code).send_keys(utils.identify_code_img(self.code_img_path))
            # ○ 点击登录后台按钮
            self.find_element(*self.btn_login_submit).click()
            # ○ 获取登录信息
