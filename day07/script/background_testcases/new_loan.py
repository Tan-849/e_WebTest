...
from background_login import driver


# 2. 打开贷款管理页面
# 2.1 定位【贷款管理】元素（上侧导航栏）
# 由于页面结构嵌套子页面，需要进入子页面才能完成元素定位
# 定位子页面元素
...
# 2.2 定位【新增贷款】元素（左侧导航栏）
# 子页面与子页面之间不能直接进行跳转，需要返回原始主页面才能再次进行切换子页面
...
# 文件上传:
#注意：由于文件上传的按钮 xpath 值可能发生变化，那么使用该 xpath 定位不到该元素，
#需要手写 xpath 进行定位
